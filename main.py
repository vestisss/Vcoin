...
def build_user_context_prompt(chat_id: int, current_user_display: str) -> dict:
    """Системный промт с именем текущего собеседника и списком участников чата."""
    mem = user_memory.get(chat_id, {})
    users = mem.get("users", {})

    lines = [f"Сейчас тебе пишет: {current_user_display}"]
    if users:
        lines.append("Участники этого чата, которых ты знаешь:")
        for uname in users.values():
            lines.append(f"  - {uname}")

    return {"role": "system", "content": "\n".join(lines)}


def build_messages(chat_id: int, username: str | None, current_user_display: str) -> list:
    mem = user_memory.setdefault(chat_id, {"messages": [], "users": {}})

    base = [
        get_system_prompt(username),
        build_user_context_prompt(chat_id, current_user_display),
    ]
...
def cmd_ban(message: types.Message):
    ...
    if not found_id and message.reply_to_message:
    ...
    if found_id:
        ...
        bot.reply_to(message, f"Не нашёл {target}. Попробуй через reply или укажи id.")
...
def handle_text(message: types.Message):
    ...
    if is_dm and uname.lower() != OWNER_USERNAME.lower():
        ...
        if count >= DM_LIMIT:
            bot.reply_to(message, "Ты достиг лимита сообщений. Напиши @neko_no_tameni и он тебя отправит нахуй.")
            return
        dm_count[user.id] = count + 1

    remember_user(chat_id, user)

    # Группа: только ! или @BotUsername
    if not is_dm:
    ...
    # Чистый текст без префикса — имя юзера идёт отдельно в системный промт
    messages = build_messages(chat_id, uname, display)
